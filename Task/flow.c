...
//			HAL_UART_Transmit(&huart4,readCmd[i],CMD_bNUM-1,1000);	//发送ID读取流量计数据
			HAL_UART_Transmit_IT(&huart4,readCmd[i],CMD_bNUM-1);	//发送ID读取流量计数据
		...
		if (COMGetDataWithTimout(COM4DataDelivery,&c))  // 接收数据解析
		{
			switch(rxState)
			{
				...
				case waitForData:
					...
					if(command_data_length>50) //未接到结束符且接收数据错误
					{
						rxState = waitForID;
						command_data_length = 0;
						tx_flag = 1;
...
void setFlow(flow_para *p ,float flow_rate)
{
	...
	HAL_UART_Transmit_IT(&huart4,alicat_init,10);	//设置流量
  while(__HAL_UART_GET_FLAG(&huart4,UART_FLAG_TC)!=SET);		//等待发送结束
	osDelay(500);   // 两个流量计之间最少500ms才能控制
}
